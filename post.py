...
class Post:
    ...
    def fetch_comments(self, chronological = False, limit = -1):
        ...
        request_data["continuation"] = response.json()["contents"]["twoColumnBrowseResultsRenderer"]["tabs"][0]["tabRenderer"]["content"]["sectionListRenderer"]["contents"][1]["itemSectionRenderer"]["contents"][0]["continuationItemRenderer"]["continuationEndpoint"]["continuationCommand"]["token"]
        response = requests.post("https://www.youtube.com/youtubei/v1/browse?prettyprint=false",
        headers = {"Content-Type": "application/json"}, data = json.dumps(request_data))
        response_data = response.json()

        # Get exact comment count from response
        self.comment_count = int(response_data["onResponseReceivedEndpoints"][0]["reloadContinuationItemsCommand"]["continuationItems"][0]["commentsHeaderRenderer"]["countText"]["runs"][0]["text"].replace(",", ""))

        # Return empty array if post has no comments
        if not "continuationItems" in response_data["onResponseReceivedEndpoints"][1]["reloadContinuationItemsCommand"]:
            return []

        # Get continuation token for correct comment sort (Top or Newest First)
        request_data["continuation"] = response_data["onResponseReceivedEndpoints"][0]["reloadContinuationItemsCommand"]["continuationItems"][0]["commentsHeaderRenderer"]["sortMenu"]["sortFilterSubMenuRenderer"]["subMenuItems"][int(chronological)]["serviceEndpoint"]["continuationCommand"]["token"]
        ...
        while limit == -1 or len(comments_data) < limit:
            ...
            if len(response_data["onResponseReceivedEndpoints"]) == 2:
                comments_data += response_data["onResponseReceivedEndpoints"][1]["reloadContinuationItemsCommand"]["continuationItems"]
            # Subsequent pages
            else:
                comments_data += response_data["onResponseReceivedEndpoints"][0]["appendContinuationItemsAction"]["continuationItems"]
            # Exit if last page
            if not "continuationItemRenderer" in comments_data[-1]:
                break

            # Grab continuation token
            request_data["continuation"] = comments_data.pop()["continuationItemRenderer"]["continuationEndpoint"]["continuationCommand"]["token"]
